...
| 呈现 companion | `human-writing` | 中文外化 / 人味改写（**视觉呈现之前**） | 吸附、改领域对错 |
| 呈现 companion | `knowledge-map` | 只读知识与学习记录，生成知识图 | 修改掌握状态、吸附 |
| 呈现 companion | `frontend-slides` | HTML 演示（宜接外化稿） | 吸附 |
| 呈现 companion | `ian-xiaohei-illustrations` | 正文配图（宜接外化稿） | 吸附 |
| 呈现 companion | `gbro-cover-design` | 封面提示词（宜接外化稿；可选 MCP 直出） | 吸附 |
...
| 「外化 / 去AI味 / 活人感 / 中文改稿」 | `human-writing` | description + `90_export` |
...
想清楚（思考 companion）
    ...
    → 【外化】中文人味改写（human-writing，可选）→ 90_export/
    → 【呈现】slides / 配图 / 封面（读外化稿或用户坚持的原稿）→ 90_export/
```
...
flowchart LR
  ...
  map --> exportDir
  domain --> outward[human-writing外化]
  outward --> exportDir
  outward --> present[视觉呈现]
  domain -->|"用户跳过外化"| present
...
呈现支路分两拍，顺序固定：**先外化（人味），再视觉呈现**。两者都进 `90_export/`，都不写 `20_领域/`。

#### 3.3-a 外化：人味改写（`human-writing`）

- 输入：`20_领域/` 或 `10_inbox/` 成稿。
- 职责：只改「怎么说」（活人感 / 去 AI 味）；不增事实；默认写出 `90_export/*-外化.md`。
- 设计要点：`skills/human-writing/docs/为什么写得好.md`；评测：`tests/eval-runs/prose-ab-2026-08-07/`。
- 交稿后**主动问一句**是否继续演示 / 配图 / 封面（可跳过）。

#### 3.3-b 视觉呈现（slides / 配图 / 封面）

- 输入：已成稿笔记、`10_inbox/` 草稿，或 **`90_export/` 里已有的外化稿**。
- **软建议（中文对外）**：用户直接要演示 / 配图 / 封面，且正文是中文知识外发时，先问一句——「要不要先用 `human-writing` 做外化改稿再呈现？」。用户说跳过或已有外化稿 → 直接呈现；**不要**不经允许强制先跑外化。
...
| 知识图 / 中文外化稿 / HTML / 配图 / 封面提示词（及可选 MCP 成品图） | `90_export/` | 只读呈现，不进吸附主路径，不修改掌握状态 |
...
- C3-c 呈现类拒绝进 `20_领域/`  
- C3-i 中文外化 human-writing → `90_export/`  
- C3-j 呈现前软建议外化  
- C3-d 脚本检查不绕过存在性  
...
| PPT/封面/外化稿进了领域树 | 呈现包 / 园丁接线失败；对照 C3-c / C3-i |
| 外化时编造了库里没有的经历 | `human-writing` 须守信息保真；对照 prose-ab 与 `docs/为什么写得好.md` |
