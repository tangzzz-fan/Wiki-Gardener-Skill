...
components:
  ...
  tree-item:
    ...
    folderGlyph: "[folder] (U+1F4C1)"
    fileGlyph: "[file] (U+1F4C4)"
    expandedMarker: "-"
    collapsedMarker: "+"
...
- Lists use a navy `>` chevron prefix; checkboxes use a literal `x` character; tree views use `[folder]` / `[file]` emoji glyphs.
...
**Tree view** (`{components.tree-item}`) — Explorer-style hierarchical list. Each row carries an expand marker (+/−), a folder (`[folder]`) or file (`[file]`) emoji glyph, and the label. 24px indentation per nesting level.
...
**Hourglass / sleep glyph** (`[hourglass]` or `[sleep]`) — Used as a single oversized character (40–52px) on splash / closing slides as a nostalgic OS-feel cue.
...
- The retro-list `>` chevron, retro-check `x`, and tree-view emoji glyphs are character-based, not vector — they will render with whatever default font metrics the runtime provides for `>`, `x`, `[folder]`, `[file]`.
