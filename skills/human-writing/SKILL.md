---
name: human-writing
description: 知识库中文外化与可读性改稿。用于用户要把已成稿笔记外化、去AI味、要活人感、中文改稿、对外发布前润色，或明确说 human-writing / 活人感写作；也适用于知乎回答、论坛长帖、公众号、博客、科普、教程、评测、口播与演讲稿。默认进 90_export/，禁止写入 20_领域/。不替代 domain-expert 执笔/审查，不替代 wiki-gardener 吸附。
license: MIT
compatibility: Companion for Wiki Gardener pack; optional vault 10_inbox/ and 90_export/; bundles check_prose.py; domain-agnostic.
metadata:
  version: "1.1.0"
  upstream: "https://github.com/KKKKhazix/human-writing"
  pair: domain-expert
  role: presenting
---
...
本包自 [KKKKhazix/human-writing](https://github.com/KKKKhazix/human-writing)（MIT）迁入，写作规则全文保留，便于本仓库统一安装与学习改调。上游版本见 `VERSION`。

## 本仓库接线（先于下文写作规则）

| 项 | 约定 |
|---|---|
| 定位 | **外化（人味）** companion：处在视觉呈现**之前**；提高对外中文读感，不是领域专家，也不是园丁 |
| 典型输入 | `20_领域/` 已吸附笔记，或用户指定的 `10_inbox/` 定稿；用户粘贴的成稿亦可 |
| 默认输出 | vault 的 `90_export/`（或用户指定导出路径）；文件名建议保留源题并加后缀如 `-外化.md` |
| 禁止 | 写入 `20_领域/`；把外化稿当成已吸附正本；擅自改 frontmatter 的 `status` / 吸附结论 |
| 与执笔分工 | 库内知识正确性、时效与 Smell → `domain-expert`；结构挂载 → `wiki-gardener`；本 skill 只改「怎么说」 |
| 信息保真 | 外化改稿不得新增事实、数字、经历、引语；材料不足则缩短或追问，禁止用想象补库内没有的经历 |
| 建议顺序 | 专家成文（+按需审查/修订）→ **本 skill 外化** → `frontend-slides` / 配图 / 封面 |
| 交到呈现 | 交稿后问一句：要不要接着做 HTML 演示 / 配图 / 封面？用户同意再交接对应呈现包，默认以其刚写出的外化稿为输入 |
| 自检 | 长稿可跑 `scripts/check_prose.py <路径>`；硬禁令清零后再交 |

对人输出禁止 emoji（本仓库 AGENTS 硬约束）。学习改调见 `docs/为什么写得好.md`。下文为上游写作规则正文。
