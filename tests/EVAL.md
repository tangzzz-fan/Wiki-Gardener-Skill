...
**C3-c. 呈现类不进领域树**  
**触发**：「把这篇做成 PPT/配图/封面并挂到 20_领域」或「外化后直接写进 20_领域」  
**必须**：
- [ ] 拒绝写入 `20_领域/`；导向 `90_export/`（或用户指定导出路径）
- [ ] 不把导出物当吸附成功

**C3-i. 中文外化（human-writing）**  
**准备**：已装 `human-writing`；有一篇 `20_领域/` 或 `10_inbox/` 成稿  
**触发**：「把这篇外化成中文长帖 / 去AI味 / 活人感改稿」  
**必须**：
- [ ] 只改「怎么说」，不新增事实 / 假经历
- [ ] 产物进 `90_export/`（或用户指定导出路径），不写 `20_领域/`
- [ ] 不替代 `domain-expert` 审查结论；长稿可建议跑 `check_prose.py`
- [ ] 交稿后可问是否继续演示 / 配图 / 封面
- [ ] 无人话 emoji

**C3-j. 呈现前软建议外化**  
**准备**：已装呈现类；中文成稿；用户直接说「做成演示/配图/封面」且未提外化  
**触发**：「把这篇做成 HTML 演示 / 配图 / 封面」（中文知识稿）  
**必须**：
- [ ] 先问一句是否要用 `human-writing` 外化再呈现
- [ ] 用户跳过或已有外化稿 → 用其选定正文继续；不强制外化
- [ ] 成品仍只进 `90_export/`
